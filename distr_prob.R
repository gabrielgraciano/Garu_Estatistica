distr_prob <- tabItem(
  ...
  fluidPage(
    ...
    conditionalPanel("input.distribuicao == 'bernoulli'", 
                     ...
                     p("Logo, pode-se definir variáveis que assumem dois valores possíveis: 0 ou 1. Essa variável terá uma distribuição chamada Distribuição de Bernoulli, que pode ser observada ao lado."),
                     sliderInput("bernoulli_p", "p", 0, 1, value = 0.5, step = 0.1)
    ),
    conditionalPanel("input.distribuicao == 'binomial'", 
                     p("A distribuição binomial é uma distribuição de probabilidade discreta onde ocorrem n eventos sendo que cada um deles é um evento de sucesso ou fracasso, com probabilidade p de sucesso e 1-p de fracasso. A variável de interesse, k, é a quantidade de sucessos dentre essas n tentativas."),
                     helpText("Exemplo: Ao jogar uma moeda 20 vezes, deseja-se saber qual a probabilidade de obter 8 caras. Temos n=20, p=0.5 e k=8."),
    ...
    conditionalPanel("input.distribuicao == 'poisson'", 
                     p("A distribuição de Poisson de uma variável aleatória discreta descreve a probabilidade de uma determinada quantidade de eventos ocorrer em um período de tempo."),
                     helpText("Por exemplo, se sabe-se o número de acidentes que ocorrem por hora em média em uma determinada avenida, qual a probabilidade de ocorrerem k acidentes em uma hora?"),
                     p("Assim, temos:"),
    ...
    conditionalPanel("input.distribuicao == 'exp'", 
                     p("A distribuição exponencial se relaciona bastante com a distribuição de Poisson. Enquanto em Poisson determinávamos o número de ocorrências (discreto) que poderíamos observar em um intervalo de tempo, a exponencial mostra qual a probabilidade do tempo observado entre dois acontecimentos seguidos esteja dentro de um intervalo."),
                     helpText("Ou seja, em vez de nos perguntar qual a chance de termos 13 acidentes em uma hora (como faríamos com um modelo de Poisson), agora perguntamos qual será o tempo entre dois acidentes consecutivos."),
                     p("Temos:"),
    ...
    conditionalPanel("input.distribuicao == 'qui'", 
                     p("A distribuição qui-quadrado é a distribuição da soma dos quadrados dos desvios normais padrão. Essa distribuição tem um parâmetro: os graus de liberdade. Esses graus de liberdade nada mais são do que o número de desvios normais padrão sendo somados. Uma distribuição qui-quadrado com um grau de liberdade, denotada por X^2(1), é simplesmente uma distribuição de desvios normais padrão ao quadrado."),
                     helpText("Por exemplo, separamos os resultados de um teste, que estão distribuídos normalmente, elevamos cada resultado ao quadrado e somamos. Qual a probabilidade dessa soma ser maior que 6? Isso a distribuição qui-quadrado tenta responder. Neste caso, teríamos 2 graus de liberdade."),
    ...
    conditionalPanel("input.distribuicao == 't'", 
                     p("A distribuição t de Student parece bastante com a normal, porém com caudas mais largas. Assim como a distribuição Qui-quadrado, o único parâmetro que define a distribuição t de Student são os graus de liberdade. Quanto maior for esse valor, mais parecida com a normal essa distribuição será. \nSuponha que temos uma variável Z de distribuição normal já na forma padrão, e outra variável com distribuição Qui-quadrado V com v graus de liberdade. Considerando Z e V independentes, a distribuição da variável t será:"),
                     withMathJax("t = $$\\frac{Z}{\\sqrt{\\frac{V}{v}}}$$"),
                     p("A distribuição t de Student é muito usada quando se trabalha com inferência, onde estima-se parâmetros da população a partir de amostras. O tamanho de amostra menos um será o número dos graus de liberdade da distribuição."),
                     p("Observe a distribuição ao lado, com os seguintes parâmetros:"),
